...
const writeFile = (mypythoncode,fname) => {
    if (fname){
        filename = fname;
    }
    let file_path = `${filename}.py`
    if (fs.existsSync(file_path)) {
        file_path = `${filename}_new.py`
    }
    fs.writeFile(file_path, mypythoncode, function (error) {
        if (error) {
            console.log(`生成代码失败:${error}`);
        } else {
            console.log(`生成Python代码成功,文件名${filename}.py`)
        }
    })
}
...
const curlcode = `
curl 'http://www.80018.cn/t.asp?/=%D7%B0%D0%DE%D6%AA%B5%C0&T=3' \\
  ...
  -H 'Referer: http://www.80018.cn/t.asp?/=%D7%B0%D0%DE%D6%AA%B5%C0&T=1' \\
  -H 'Accept-Language: zh-CN,zh;q=0.9' \\
  -H 'Cookie: ASPSESSIONIDCQQRRQQA=LCOKIKIDNMEBGBALKKMBBFNE' \\
  --compressed \\
  --insecure
`
const mypythoncode = toPython(curlcode)
writeFile(mypythoncode,)